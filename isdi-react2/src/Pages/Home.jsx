...
export default function Home({ onLoggedOut }) {
    ...
    const handleViewLogout = () => {
        if (hideLogoutLink) {
            hideLogoutLink.style.display = !isMenuOpen ? 'none' : 'flex'
        }

        if (hideAvatarLink) {
            hideAvatarLink.style.display = !isMenuOpen ? 'none' : 'flex'
        }
    }
    ...
    const handleSwitchMode = () => {
        ...
        if (document.querySelector(':root').classList.contains('dark')) {
            document.querySelector('.btn-home-dark-theme').innerHTML = <FiSun />
        } else {
